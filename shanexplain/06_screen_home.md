...
> **Files:** `lib/shell/main_shell.dart`, `lib/features/home/screens/home_tab.dart`, `lib/features/home/screens/food_tab.dart`
> **Category:** screen
> **Added:** 2026-04-29
> **Related files:** `navigation_viewmodel.dart`, `auth_viewmodel.dart`, `profile_drawer.dart`, `13_shell_main.md`, `14_ui_profile_drawer.md`
...
- **Food tab** (`FoodTab`) — shows a placeholder "Empty" message. Real content coming later.
...
2. **`MainShell` reads `NavigationViewModel.currentIndex`** — this tells it which tab is currently selected (0 = Home, 1 = Food).
3. **The `Scaffold` is built** with three pieces:
   ...
   - `body:` — `_tabs[currentIndex]`, which is either `HomeTab()` or `FoodTab()`.
   - `bottomNavigationBar:` — The `NavigationBar` with the two destination buttons.
...
7. **`FoodTab` renders** — just shows the word "Empty" for now.
...
| `_tabs[currentIndex]` | A list of two widgets — accessing index 0 gives `HomeTab`, index 1 gives `FoodTab` |
