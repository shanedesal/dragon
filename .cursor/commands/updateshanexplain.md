...
Determine which files changed: run `git diff --name-only` and `git diff --cached --name-only` (merge/dedupe the lists), or use `git status --short`. Include any paths the user attached in chat.
